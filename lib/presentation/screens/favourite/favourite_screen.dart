...
class FavouriteScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<FavoriteBloc>(param1: FailureMapper(context)),
      child: const _FavouriteScreenView(),
    );
  }
}

class _FavouriteScreenView extends StatelessWidget {
                ...
                children: [
                  Flexible(
                    child: ListView.separated(
                      ...
                      itemCount:
                          state.favoriteItems?.listOfFavoriteItems.length ?? 0,
                      itemBuilder: (context, index) {
                        final item =
                            state.favoriteItems?.listOfFavoriteItems[index];
                        return Padding(
          ...
          if (state.isLoading) {
            ...
            if (state.apiError != null) {
              CommonDialogs.showErrorDialog(
                ...
                onTap: () {
                  context.read<FavoriteBloc>().add(OnClearErrorEvent());
                },
              );
            }
          }
